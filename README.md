...
> **本项目冻结声明**
>
> 目前我还尚未把“一个可靠的 DFT 研究者究竟如何工作”理解和表达清楚，因此现在直接自动化，极易把错误抽象固化进代码。所以当前项目决定先搁置一段时间，避免把项目总体带偏。DFT 研究的流程是非常繁琐的，任何的一个细微操作都可以会导致结果出现偏移，所以如果追求一个可以省心的自动化DFT流程，可能需要先对DFT的每个操作做一些严格的理解、讨论。目前项目可能初步可用了，能够提高Agent的DFT严谨性和效率，但是其实更像是一个demo。在AI4DFT的路上，越走越发现DFT是一个流程极度繁琐，甚至无法做到脱离人工审查的工作，所以我觉得还是搁置一段时间，完整规划之后再重启这个项目好一些。
