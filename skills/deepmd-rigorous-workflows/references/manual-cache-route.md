...
```bash
python3 tools/sync_official_manual_cache.py --refresh --skill deepmd-rigorous-workflows
...
Read the generated `index.md` under `${XDG_CACHE_HOME:-$HOME/.cache}/vibe-dft-skills/official-manuals/deepmd-rigorous-workflows/`. Keep the official URL, authority/version identity, and local conversion label in every claim. A cache pass proves document identity and readability only; it does not prove executable behavior, convergence, physical validity, or permission to redistribute the cached body.

This Skill has 3 registered source records across these authorities:

- `deepmd-release-source-docs-3-1-3`
- `dpdata-release-source-docs-1-0-2`
- `dpgen-release-source-docs-0-13-3`
