...
import review_response_cli as cli
import validate_candidate_decision_table as decision_validator


class ReviewResponseCliTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def request(self) -> dict:
        return json.loads(self.fixture("valid-review-request.json").read_text(encoding="utf-8"))

    def claim_map(self) -> dict:
        return json.loads(self.fixture("valid-review-claim-map.json").read_text(encoding="utf-8"))
    ...
    def plan(self, request_path: Path | None = None, map_path: Path | None = None, name: str = "plan.json") -> tuple[int, dict, Path]:
        output = self.work / name
        code, summary = self.invoke([
            "plan", "--request", str(request_path or self.fixture("valid-review-request.json")),
            "--claim-map", str(map_path or self.fixture("valid-review-claim-map.json")),
            "--out", str(output),
        ])
    ...
    def modified_map_and_request(self, mutate) -> tuple[Path, Path]:
        ...
        return self.write_json("request.json", request), map_path

    def test_strict_json_rejects_duplicate_bom_nan_and_concatenation(self) -> None:
        for index, raw in enumerate((b'{"a":1,"a":2}', b'\xef\xbb\xbf{}', b'{"x":NaN}', b'{}{}')):
...
"""
    ...
    def test_valid_plan_is_stable_and_no_send(self) -> None:
        ...
        self.assertFalse(plan["manuscript_modified"])
        self.assertFalse(plan["external_message_sent"])
        self.assertFalse(plan["submission_ready"])
        self.assertNotIn("text", plan["comments"][0])
        self.assertNotIn("requested_action", plan["comments"][0])

    def test_claim_map_hash_mismatch_is_invalid(self) -> None:
        request = self.request()
        request["claim_map_ref"]["sha256"] = "f" * 64
        code, summary, _ = self.plan(self.write_json("request.json", request))
        self.assertEqual(code, 2)
        self.assertIn("REVIEW.HASH.MISMATCH", summary["finding_codes"])

    def test_missing_response_blocks_comment_coverage(self) -> None:
        request = self.request()
        request["responses"] = []
        code, summary, _ = self.plan(self.write_json("request.json", request))
        self.assertEqual(code, 3)
        self.assertIn("REVIEW.COMMENT.COVERAGE_INVALID", summary["finding_codes"])

    def test_duplicate_comment_is_invalid(self) -> None:
        request = self.request()
        request["comments"].append(dict(request["comments"][0]))
        code, summary, _ = self.plan(self.write_json("request.json", request))
        self.assertEqual(code, 2)
        self.assertIn("REVIEW.IDENTITY.DUPLICATE", summary["finding_codes"])

    def test_unknown_change_is_invalid(self) -> None:
        request = self.request()
        request["responses"][0]["change_ids"] = ["missing-change"]
        code, summary, _ = self.plan(self.write_json("request.json", request))
        self.assertEqual(code, 2)
        self.assertIn("REVIEW.RESPONSE.CHANGE_LINK_INVALID", summary["finding_codes"])

    def test_missing_no_change_reason_is_invalid(self) -> None:
        request = self.request()
        request["responses"][0]["change_ids"] = []
        request["responses"][0]["no_change_reason"] = None
        code, summary, _ = self.plan(self.write_json("request.json", request))
        self.assertEqual(code, 2)
        self.assertIn("REVIEW.RESPONSE.CHANGE_LINK_INVALID", summary["finding_codes"])

    def test_unsupported_claim_blocks_response(self) -> None:
        def mutate(value: dict) -> None:
            value["claims"][0]["status"] = "blocked"
            value["claims"][0]["blocker_codes"] = ["synthetic-blocker"]

        request_path, map_path = self.modified_map_and_request(mutate)
        code, summary, _ = self.plan(request_path, map_path)
        self.assertEqual(code, 3)
        self.assertIn("REVIEW.CLAIM.UNSUPPORTED", summary["finding_codes"])

    def test_missing_evidence_blocks_response(self) -> None:
        def mutate(value: dict) -> None:
        ...
        code, summary, _ = self.plan(request_path, map_path)
        self.assertEqual(code, 3)
        self.assertIn("REVIEW.RESPONSE.EVIDENCE_MISSING", summary["finding_codes"])

    def test_nonpassing_claim_gate_blocks_response(self) -> None:
        def mutate(value: dict) -> None:
        ...
        code, summary, _ = self.plan(request_path, map_path)
        self.assertEqual(code, 3)
        self.assertIn("REVIEW.CLAIM.GATE_NOT_PASSING", summary["finding_codes"])

    def test_thin_self_declared_gate_is_invalid(self) -> None:
        def mutate(value: dict) -> None:
            value["gates"][0] = {"check_id": "technical-completion", "status": "pass"}

        request_path, map_path = self.modified_map_and_request(mutate)
        code, summary, _ = self.plan(request_path, map_path)
        self.assertEqual(code, 2)
        self.assertIn("REVIEW.GATE.SHAPE_INVALID", summary["finding_codes"])

    def test_implemented_change_without_artifact_blocks(self) -> None:
        request = self.request()
        request["modifications"][0]["status"] = "implemented"
        code, summary, _ = self.plan(self.write_json("request.json", request))
        self.assertEqual(code, 2)
        self.assertIn("REVIEW.CHANGE.IMPLEMENTATION_UNVERIFIED", summary["finding_codes"])
        self.assertFalse((self.work / "plan.json").exists())

    def test_private_path_is_invalid(self) -> None:
        request = self.request()
        request["comments"][0]["text"] = "Inspect /" + "scratch/example/result"
        code, summary, _ = self.plan(self.write_json("request.json", request))
        self.assertEqual(code, 2)
        self.assertIn("REVIEW.PRIVACY.UNSAFE_TEXT", summary["finding_codes"])
        self.assertFalse((self.work / "plan.json").exists())

    def test_positive_upstream_claim_requires_external_bundle_and_is_redacted(self) -> None:
        sentinel = "SENTINEL_POSITIVE_REVIEW_ASSERTION"

        def mutate(value: dict) -> None:
            value["claim_ceiling"] = "technical_run_gates_only"
            value["claims"][0]["claim_level"] = "technical_run_gates_only"
            value["claims"][0]["statement"] = sentinel

        request_path, map_path = self.modified_map_and_request(mutate)
        code, summary, output = self.plan(request_path, map_path, "positive-plan.json")
        self.assertEqual(code, 3)
        self.assertEqual(summary["first_decisive_finding"], "REVIEW.CLAIM.EXTERNAL_BUNDLE_REQUIRED")
    ...
    def test_audit_and_render_bind_exact_plan(self) -> None:
        ...
        self.assertEqual(len(value["point_by_point"]), 1)
        self.assertFalse(value["external_message_sent"])
        serialized = package.read_text(encoding="utf-8")
        self.assertNotIn("Please clarify how technical completion is evidenced.", serialized)
        self.assertNotIn("Add an evidence-bound clarification.", serialized)
    ...
    def test_stale_audit_blocks_render(self) -> None:
        ...
        self.assertEqual(code, 3)
        self.assertIn("REVIEW.RENDER.AUDIT_MISMATCH", summary["finding_codes"])
    ...
    def test_forged_pass_audit_cannot_authorize_modified_plan(self) -> None:
        ...
        self.assertEqual(code, 3)
        self.assertEqual(summary["first_decisive_finding"], "REVIEW.RENDER.AUDIT_MISMATCH")
        self.assertFalse(output.exists())

    def test_audit_rejects_forged_plan_with_raw_reviewer_text(self) -> None:
        plan, _audit = self.valid_plan_and_audit()
        value = json.loads(plan.read_text(encoding="utf-8"))
        value["comments"][0]["text"] = "SENTINEL_FORGED_REVIEWER_TEXT"
        forged_plan = self.write_json("raw-comment-forged-plan.json", value)
        audit_out = self.work / "raw-comment-forged-audit.json"
        code, summary = self.invoke(["audit", "--plan", str(forged_plan), "--out", str(audit_out)])
        self.assertEqual(code, 2)
        self.assertIn("REVIEW.COMMENT.INVALID", summary["finding_codes"])
        self.assertFalse(audit_out.exists())

    def test_change_response_evidence_claim_chain_is_one_to_one(self) -> None:
        request = self.request()
        duplicate = dict(request["modifications"][0])
        duplicate["change_id"] = "change-002"
        duplicate["comment_ids"] = list(duplicate["comment_ids"])
        duplicate["evidence_ids"] = list(duplicate["evidence_ids"])
        duplicate["claim_ids"] = list(duplicate["claim_ids"])
        duplicate["limitations"] = list(duplicate["limitations"])
        request["modifications"].append(duplicate)
        request["responses"][0]["change_ids"] = ["change-001", "change-002"]
        code, summary, output = self.plan(self.write_json("many-change-request.json", request))
        self.assertEqual(code, 2)
        self.assertEqual(summary["first_decisive_finding"], "REVIEW.RESPONSE.CHANGE_LINK_INVALID")
        self.assertFalse(output.exists())

    def test_output_cannot_alias_any_input(self) -> None:
        request_path = self.fixture("valid-review-request.json")
        map_path = self.fixture("valid-review-claim-map.json")
        for output in (request_path, map_path):
            ...
            self.assertEqual(code, 2)
            self.assertIn("REVIEW.JSON.INVALID", summary["finding_codes"])
    ...
    def test_output_overwrite_is_rejected(self) -> None:
        ...
        code, summary = self.invoke([
            "plan", "--request", str(self.fixture("valid-review-request.json")),
            "--claim-map", str(self.fixture("valid-review-claim-map.json")), "--out", str(output),
        ])
        self.assertEqual(code, 2)
        self.assertIn("REVIEW.JSON.INVALID", summary["finding_codes"])
    ...
    def test_canonical_weak_model_decision_table_and_executable_cases(self) -> None:
        ...
        self.assertEqual(decision_validator.validate_file(path, expected_skill_id="dft-review-response"), [])
        ...
        for case in table["cases"]:
            with self.subTest(case_id=case["case_id"]):
                ...
                if case["case_id"] in {"review-unsafe-input-identity", table["default_case_id"]}:
                    continue
                request = self.request()
                claim_map = self.claim_map()
                scenario = case["case_id"]
                if scenario == "review-comment-unanswered":
                    request["responses"] = []
                elif scenario == "review-comment-double-answered":
                    duplicate = copy.deepcopy(request["responses"][0])
                    duplicate["response_id"] = "response-002"
                    request["responses"].append(duplicate)
                elif scenario == "review-change-unknown":
                    request["responses"][0]["change_ids"] = ["missing-change"]
                elif scenario == "review-response-evidence-missing":
                    request["responses"][0]["evidence_ids"] = []
                elif scenario == "review-claim-unsupported":
                    claim_map["claims"][0]["status"] = "blocked"
                    claim_map["claims"][0]["blocker_codes"] = ["synthetic-blocker"]
                elif scenario == "review-claim-gate-blocked":
                    claim_map["gates"][0]["status"] = "blocked"
                elif scenario == "review-change-claims-completed":
                    request["modifications"][0]["status"] = "completed"
                elif scenario == "review-change-orphaned":
                    orphan = copy.deepcopy(request["modifications"][0])
                    orphan["change_id"] = "change-002"
                    request["modifications"].append(orphan)
                elif scenario == "review-positive-upstream-claim":
                    claim_map["claim_ceiling"] = "technical_run_gates_only"
                    claim_map["claims"][0]["claim_level"] = "technical_run_gates_only"
                elif scenario == "review-private-comment":
                    request["comments"][0]["text"] = "inspect /scratch/example/private"
                elif scenario == "review-multiple-changes-for-response":
                    duplicate = copy.deepcopy(request["modifications"][0])
                    duplicate["change_id"] = "change-002"
                    request["modifications"].append(duplicate)
                    request["responses"][0]["change_ids"] = ["change-001", "change-002"]
                elif scenario == "review-thin-gate":
                    claim_map["gates"][0] = {"check_id": "technical-completion", "status": "pass"}
                elif scenario not in {"review-valid-one-to-one", "review-hash-mismatch"}:
                    self.fail(f"unhandled canonical scenario: {scenario}")
                map_path = self.write_json(f"{case['case_id']}-map.json", claim_map)
                _value, digest = cli.strict_load(map_path)
                request["claim_map_ref"]["sha256"] = "f" * 64 if scenario == "review-hash-mismatch" else digest
    ...
    def test_finding_catalog_exhaustively_matches_cli_literals(self) -> None:
        ...
        discovered = {}
        tree = ast.parse((ROOT / "scripts" / "review_response_cli.py").read_text(encoding="utf-8"))
...
class OfficialSourcePackMetadataTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.catalog_path = self.repository / self.seed["providers"][0]["source_ref"]["path"]
    ...
    def test_source_pack_matches_schemas_and_exact_refs(self) -> None:
        ...
        for ref in (
            ...
            self.seed["providers"][0]["source_ref"],
        ):
    ...
    def test_repository_carrier_cannot_replace_runtime_evidence_authorities(self) -> None:
        ...
        universal = next(
            subject
            for subject in self.scope["subjects"]
            if subject["subject_id"] == "review.authority.universal-provider-excluded"
        )
        self.assertEqual("excluded", universal["expected_disposition"])
    ...
    def test_metadata_only_proposal_and_generator_are_fail_closed(self) -> None:
        ...
        authority = proposal["authority_entries"][
            "vibe-dft-skills-repository-contracts"
        ]
        self.assertEqual("repository", authority["provider_class"])
