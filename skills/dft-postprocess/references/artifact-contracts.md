...
`contracts/structure-manifest.schema.json` is the upstream CIF/structure-inspection contract and can be checked with `dftpost validate-manifest structure`. It is not a postprocessing artifact manifest and does not by itself prove that a calculation used that structure; a future run-manifest structure reference must bind the manifest ID, source hash, structure fingerprint, and terminal transformation.
