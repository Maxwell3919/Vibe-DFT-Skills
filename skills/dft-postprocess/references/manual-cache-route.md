...
```bash
python3 tools/sync_official_manual_cache.py --refresh --skill dft-postprocess
...
Read the generated `index.md` under `${XDG_CACHE_HOME:-$HOME/.cache}/vibe-dft-skills/official-manuals/dft-postprocess/`. Keep the official URL, authority/version identity, and local conversion label in every claim. A cache pass proves document identity and readability only; it does not prove executable behavior, convergence, physical validity, or permission to redistribute the cached body.

This Skill has 25 registered source records across these authorities:

- `ase-release-source-docs-3-29-0`
- `bader-1-05-official-page`
- `cp2k-official-manual`
- `qe-official-docs`
- `siesta-release-source-docs`
- `vasp-official-wiki`
- `vaspkit-1-5-documentation`
- `vesta-official-manual`
