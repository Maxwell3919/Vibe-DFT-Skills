...
If status is `blocked_local_official_source`, inspect `missing` and `corrupt`, then check the matching live official VASP Wiki page. If live verification is unavailable, mark the software-behavior claim unresolved. Missing or hash-invalid mirror evidence never authorizes a remembered answer.
...
```bash
python3 scripts/audit_vasp_case.py CASE \
  --mode run --task-type TASK --pretty > vasp_run_audit.json
```
...
| Local source coverage is partial | Some tags have provenance-bearing local official pages | Missing tags are undocumented by VASP |
