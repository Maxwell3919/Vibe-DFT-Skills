...
```bash
python3 tools/sync_official_manual_cache.py --refresh --skill vasp-rigorous-calculations
...
```bash
python3 skills/vasp-rigorous-calculations/scripts/sync_official_wiki.py --refresh
python3 skills/vasp-rigorous-calculations/scripts/sync_official_wiki.py --check
```
...
Read the generated `index.md` under `${XDG_CACHE_HOME:-$HOME/.cache}/vibe-dft-skills/official-manuals/vasp-rigorous-calculations/`. Keep the official URL, authority/version identity, and local conversion label in every claim. A cache pass proves document identity and readability only; it does not prove executable behavior, convergence, or physical validity.

This Skill has 162 registered source records across these authorities:
