...
- Official URL: https://www.vasp.at/wiki/LEPSILON
- Page ID: 119
- Revision ID: 34646
...
Determines the static ion-clamped dielectric matrix using density functional perturbation theory (DFPT). The dielectric matrix is calculated with and without local field effects. Usually, local field effects are determined on the Hartree level, i.e. including changes of the Hartree potential.
To include microscopic changes of the exchange-correlation potential
the tag LRPA=.FALSE. must be set. The method is explained in detail in paper by Gajdoš et al. [1] and closely follows the original work of Baroni and Resta [2].
