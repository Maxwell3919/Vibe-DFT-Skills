...
- Official URL: https://www.vasp.at/wiki/KPOINTS
- Page ID: 1389
- Revision ID: 35974
...
- 2 Explicit k-point mesh

- 3 Regular k-point mesh

- 4 Symmetry reduction of the mesh

- 5 Generalized regular meshes

- 6 Band-structure calculations

- 7 Automatic k-point mesh

- 8 Related tags and sections
...
The k points are linear combinations of the reciprocal lattice vectors [math]\displaystyle{ {\mathbf b}_{1\ldots3} }[/math]
[math]\displaystyle{ {\mathbf k} = x_1 {\mathbf b}_1 + x_2 {\mathbf b}_2 + x_3 {\mathbf b}_3~. }[/math]
Use the factors [math]\displaystyle{ x_{1\ldots3} }[/math] as the coordinates in KPOINTS.
Cartesian coordinate system
The coordinates [math]\displaystyle{ x_{1\ldots3} }[/math] directly correspond to the k point
[math]\displaystyle{ {\mathbf k} =\frac{2 \pi}{a} (x_1, x_2 , x_3)~. }[/math]
up to the scaling factor [math]\displaystyle{ 2\pi / a }[/math]. Here, [math]\displaystyle{ a }[/math] is the scaling parameter specified on the second line of the POSCAR file.

Example: face-centered-cubic (fcc) lattice

The following lattice vectors [math]\displaystyle{ \mathbf a_i }[/math] span the unit cell:
...
The corresponding reciprocal lattice vectors [math]\displaystyle{ \mathbf b_i }[/math] are
...
Explicit k-point mesh[edit | edit source]
...
Regular k-point mesh[edit | edit source]
...
Choose the number of subdivisions [math]\displaystyle{ N_1 }[/math], [math]\displaystyle{ N_2 }[/math] and [math]\displaystyle{ N_3 }[/math] in the KPOINTS file like this
...
- Specify the desired number of subdivisions [math]\displaystyle{ N_1 }[/math], [math]\displaystyle{ N_2 }[/math] and [math]\displaystyle{ N_3 }[/math] in the fourth line.

- Optionally add a fifth line to shift the mesh by [math]\displaystyle{ (s_1, s_2, s_3) }[/math] with respect to the default.

Γ-centered mesh
The following k points sample the Brillouin zone
[math]\displaystyle{ {\mathbf k} = \sum_{i = 1}^3 \frac{n_i+s_i}{N_i} {\mathbf b}_i \qquad \forall {n_i \in [0, N_i[} }[/math]
Monkhorst-Pack mesh
The k point mesh results from this definition
[math]\displaystyle{ {\mathbf k} = \sum_{i = 1}^3 \frac{n_i+s_i+\frac{1-N_i}{2}}{N_i} {\mathbf b}_i \qquad \forall {n_i \in [0, N_i[} }[/math]

The spacing between the points is the same for both meshes.
The only difference is the shift [math]\displaystyle{ (1-N_i)/2 }[/math] in the numerator of the Monkhorst-Pack mesh.
For an odd number of subdivisions, this term is an integer, and therefore the two meshes agree due to periodic boundaries.
When the number of subdivisions is even the Γ-centered mesh is shifted by [math]\displaystyle{ s_i = 1/2 }[/math] compared to the Monkhorst-Pack one.
...
As a rule of thumb, choose [math]\displaystyle{ N_1 }[/math], [math]\displaystyle{ N_2 }[/math], and [math]\displaystyle{ N_3 }[/math] such that

[math]\displaystyle{ N_1 : N_2 : N_3 \approx |\mathbf b_1| : |\mathbf b_2| : |\mathbf b_3|~. }[/math]

This guideline is also implemented for the automatic k-point generation using KSPACING.
Nevertheless, specifying the [math]\displaystyle{ N_1 }[/math], [math]\displaystyle{ N_2 }[/math], and [math]\displaystyle{ N_3 }[/math] manually ensures that changes in the lattice vectors do not affect the k-point mesh.
When the primitive cell has (nearly) perpendicular axes (cubic, tetragonal, orthorhombic), this is equivalent to:

[math]\displaystyle{ N_1 : N_2 : N_3 \approx \frac{1}{|\mathbf a_1|} : \frac{1}{|\mathbf a_2|} : \frac{1}{|\mathbf a_3|}~. }[/math]
...
Symmetry reduction of the mesh[edit | edit source]

VASP determines the symmetry of the system.
For ISYM [math]\displaystyle{ \ge }[/math] 0, the automatically generated k-point meshes are reduced to the irreducible subset.
...
Important: To enable an efficient symmetry reduction, the (shifted) regular mesh of k points should conserve the point-group symmetry of the reciprocal lattice. Specifically, the generating lattice ([math]\displaystyle{ \mathbf g_i = \mathbf k_i / N_i }[/math]) should belong to the same class of Bravais lattice as the reciprocal lattice.
...
[math]\displaystyle{ \Gamma }[/math]-centered, Monkhorst Pack

[math]\displaystyle{ |\mathbf a_1|^{-1} : |\mathbf a_2|^{-1} : |\mathbf a_3|^{-1} }[/math], [math]\displaystyle{ |\mathbf b_1| : |\mathbf b_2| : |\mathbf b_3| }[/math]
...
[math]\displaystyle{ |\mathbf a_1|^{-1} : |\mathbf a_2|^{-1} : |\mathbf a_3|^{-1} }[/math]

face-centered

[math]\displaystyle{ \Gamma }[/math]-centered

[math]\displaystyle{ |\mathbf b_1| : |\mathbf b_2| : |\mathbf b_3| }[/math]
...
- Use only [math]\displaystyle{ \Gamma }[/math]-centered meshes for face-centered cubic (fcc), hexagonal, and fcc-orthorhombic crystalline lattices.

- Choose the ratios of the subdivisions [math]\displaystyle{ N_1 : N_2 : N_3 = |\mathbf a_1|^{-1} : |\mathbf a_2|^{-1} : |\mathbf a_3|^{-1} }[/math] for body-centered tetragonal and body-centered orthorhombic lattices. Keep in mind that the KSPACING uses the reciprocal lattice vectors so may not be suitable for these symmetries.

- For face-centered orthorhombic crystal structures, choose subdivisions according to [math]\displaystyle{ N_1 : N_2 : N_3 = |\mathbf b_1| : |\mathbf b_2| : |\mathbf b_3| }[/math].
...
Generalized regular meshes[edit | edit source]
...
VASP generates three vectors [math]\displaystyle{ \mathbf g_1 }[/math], [math]\displaystyle{ \mathbf g_2 }[/math], and [math]\displaystyle{ \mathbf g_3 }[/math] from the coefficients [math]\displaystyle{ x_i }[/math] given in line 4–6.
Depending on the selected coordinate system these vectors are either multiples of the reciprocal lattice vectors
[math]\displaystyle{ \mathbf b_i }[/math] (reciprocal) or simply multiplying the coefficients by [math]\displaystyle{ 2\pi/a }[/math] (Cartesian).
Here [math]\displaystyle{ a }[/math] is the scaling parameter you have specified on the second line of the POSCAR file.

Important: The generating vectors [math]\displaystyle{ \mathbf g_1 }[/math], [math]\displaystyle{ \mathbf g_2 }[/math], and [math]\displaystyle{ \mathbf g_3 }[/math] must be commensurate with the reciprocal lattice. This means [math]\displaystyle{ \mathbf b_i = \textstyle \sum_{j} M_{ij} \mathbf g_j }[/math] where the matrix [math]\displaystyle{ M }[/math] contains only integer entries. If this is not the case the code will exit in error.

Combined with the shift ([math]\displaystyle{ s_1 }[/math], [math]\displaystyle{ s_2 }[/math], [math]\displaystyle{ s_3 }[/math]) specified in the last line, VASP uses the generating vectors [math]\displaystyle{ \mathbf g_i }[/math] to construct the k-point mesh

[math]\displaystyle{ {\mathbf k} = \sum_i {\mathbf g}_i (n_i + s_i) \qquad n_i \in [0, N_i[ }[/math]

where VASP chooses the [math]\displaystyle{ N_i }[/math] to include all possible points of the generating mesh in the first Brillouin zone.

The regular k-point meshes are a subset of the generalized regular meshes, for which

[math]\displaystyle{ \mathbf g_i = \mathbf b_i / N_i~. }[/math]

Here, the generating lattice vectors are integer subdivisions of the reciprocal lattice vectors according to the [math]\displaystyle{ N_i }[/math] defined in the KPOINTS file.
...
where the rows of [math]\displaystyle{ A }[/math] represent the lattice vector of the primitive cell.
...
yields a roughly uniform sampling of the Brillouin zone when [math]\displaystyle{ N_1 }[/math], [math]\displaystyle{ N_2 }[/math], and [math]\displaystyle{ N_3 }[/math] are chosen such that:

[math]\displaystyle{ N_1 : N_2 : N_3 \approx 1 : \frac{a}{b} : \frac{a}{c} }[/math]

For instance, for a body-centered orthorhombic primitive cell with [math]\displaystyle{ a=5, \; b/a=1.2 \; c/a=0.5 }[/math], here given in POSCAR file format:
...
corresponds to the aforementioned generating lattice for [math]\displaystyle{ N_1=2 }[/math], [math]\displaystyle{ N_2=2 }[/math], and [math]\displaystyle{ N_3=4 }[/math].

Furthermore, using generalized regular meshes potentially requires fewer k points compared to Monkhorst-Pack meshes to converge total energy calculations.[3][4]
...
But external tools[5][6] construct meshes with certain target sampling density in the spirit of the aforementioned publications.

Band-structure calculations[edit | edit source]
...
Specifically for the template above, 40 points from [math]\displaystyle{ \Gamma }[/math] to X, 40 points from X to W, and 40 points from W to [math]\displaystyle{ \Gamma }[/math].
...
External tools[1][2] are useful to decide which paths in the Brillouin zone to include.
...
Automatic k-point mesh[edit | edit source]

Deprecated: The KSPACING tag provides almost the same functionality. Preferably use that method instead.

The following KPOINTS file generates a regular [math]\displaystyle{ \Gamma }[/math]-centered k-point.
The subdivisions [math]\displaystyle{ N_1 }[/math], [math]\displaystyle{ N_2 }[/math], and [math]\displaystyle{ N_3 }[/math] are along the reciprocal lattice vectors [math]\displaystyle{ \mathbf b_1 }[/math], [math]\displaystyle{ \mathbf b_2 }[/math], and [math]\displaystyle{ \mathbf b_3 }[/math], respectively.
...
- The fourth line defines a length ([math]\displaystyle{ R_k }[/math]) that determines the subdivisions [math]\displaystyle{ N_1 }[/math], [math]\displaystyle{ N_2 }[/math], and [math]\displaystyle{ N_3 }[/math].

For every lattice vector [math]\displaystyle{ \mathbf b_i }[/math] the number of subdivisions is calculated as

[math]\displaystyle{ N_i = \text{int}\left(\max(1, R_k |{\mathbf b}_i| + 0.5)\right)~. }[/math]

Note that this similar to the KSPACING tag, when the length [math]\displaystyle{ R_k = 2\pi/\text{KSPACING} }[/math].
The generated mesh is centered at [math]\displaystyle{ \Gamma }[/math]

[math]\displaystyle{ {\mathbf k} = \sum_{i=1}^3 {\mathbf b}_i \frac{n_i}{N_i} \qquad \forall n_i \in [0, N_i[ }[/math]

Useful values for the length vary between [math]\displaystyle{ R_k = 10 }[/math] (large gap insulators) and [math]\displaystyle{ R_k = 100 }[/math] (d metals).
Please verify that changes to [math]\displaystyle{ R_k }[/math] do not affect the quantity of interest.
...
Related tags and sections[edit | edit source]

KSPACING, KPOINTS_OPT, IBZKPT, Number of k points and method for smearing

References[edit | edit source]

- ↑ a b www.cryst.ehu.es/cryst/get_kvec.html (2022).

- ↑ a b www.materialscloud.org/work/tools/seekpath (2022).

- ↑ a b P. Wisesa, K.A. McGill, and T. Mueller, Phys. Rev. B 93, 155109 (2016).

- ↑ a b W.S. Morgan, J.E. Christensen, P.K. Hamilton, J.J. Jorgensen, B.J. Campbell, G.L.W. Hart, and R.W. Forcade, Comput. Mater. Sci. 173, 109340 (2020).

- ↑ a b https://muellergroup.jhu.edu/K-Points.html (2022).

- ↑ a b https://github.com/msg-byu/autoGR (2022).
