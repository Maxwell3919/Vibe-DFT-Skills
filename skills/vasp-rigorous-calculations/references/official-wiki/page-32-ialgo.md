...
- Official URL: https://www.vasp.at/wiki/IALGO
- Page ID: 32
- Revision ID: 29456
...
for VASP.4.4 and older

= 38

else (if ALGO is not set)
...
Conjugate gradient algorithms[edit | edit source]
...
The blocked-Davidson scheme[edit | edit source]
...
The RMM-DIIS algorithm[1][2] reduces the number of orthonormalization steps ([math]\displaystyle{ O(N^3) }[/math]) considerably and is therefore much faster than IALGO=8 and IALGO=38, at least for large systems and for workstations with a small memory band width. For optimal performance, we recommend to use this switch together with LREAL=Auto). The algorithm works in a blocked mode in which several bands are optimized at the same time. This can improve the performance even further on systems with a low memory band width (default is presently NSIM=4).
