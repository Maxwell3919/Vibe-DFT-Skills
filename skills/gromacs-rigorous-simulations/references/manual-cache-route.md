...
```bash
python3 tools/sync_official_manual_cache.py --refresh --skill gromacs-rigorous-simulations
...
Read the generated `index.md` under `${XDG_CACHE_HOME:-$HOME/.cache}/vibe-dft-skills/official-manuals/gromacs-rigorous-simulations/`. Keep the official URL, authority/version identity, and local conversion label in every claim. A cache pass proves document identity and readability only; it does not prove executable behavior, convergence, physical validity, or permission to redistribute the cached body.

This Skill has 1 registered source records across these authorities:

- `gromacs-release-source-docs-2026-3`
