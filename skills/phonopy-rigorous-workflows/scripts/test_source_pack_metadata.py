#!/usr/bin/env python3
"""Offline integrity tests for phonopy-rigorous-workflows source-pack inputs."""
...
class SourcePackMetadataTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.catalog_path = REFERENCES / "source-pack-source-catalog-phonopy.json"
    ...
    def test_seed_hashes_are_local_and_exact(self) -> None:
        ...
        self.assertEqual("phonopy-docs", provider["input_id"])
        self.assertEqual("phonopy-release-source-docs-4-3-1", provider["authority_id"])
    ...
    def test_semantic_scope_preserves_all_provider_subjects(self) -> None:
        ...
        for item in self.scope["subjects"]:
            if item["evidence_class"] == "official-provider-required":
                ...
                self.assertEqual(["phonopy-docs"], item["provider_input_ids"])
            else:
    ...
    def test_tree_inventory_is_exact_and_path_complete(self) -> None:
        inventory = load(REFERENCES / "source-pack-inventory-phonopy.json")
