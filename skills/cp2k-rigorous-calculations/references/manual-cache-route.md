...
```bash
python3 skills/cp2k-rigorous-calculations/scripts/sync_official_manuals.py --refresh
python3 skills/cp2k-rigorous-calculations/scripts/sync_official_manuals.py --check
```
...
Read the generated `index.md` under `${XDG_CACHE_HOME:-$HOME/.cache}/vibe-dft-skills/official-manuals/cp2k-rigorous-calculations/`. Keep the official URL, authority/version identity, and local conversion label in every claim. A cache pass proves document identity and readability only; it does not prove executable behavior, convergence, or physical validity.
