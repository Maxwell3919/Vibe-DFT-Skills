...
is a method that allows the acceleration of rare events and estimation of the free
...
molecules at the rutile (110)

\(TiO_2\)

surface. For more information about the keywords used in the
...
obtained from DFT-based Born-Oppenheimer MD simulations;
...
, with some water molecules co-adsorbed and some others in the vicinity of the

system.

\(HCOO^-\)

is bound either in a bridged, bidentate way to surface titanium atoms or in a
...
simulation together with the definition of the relevant
...
NN and ND determine the curvature of the function used to compute the CN, and
...
section controls the printing of
...
has, in principle, different dimensions and dynamics, the shape of the hills filling up the
...
space-direction of the

\(N_{CV}\)

-dimensional configuration space. All three parameters (WW,
...
available with cp2k, in the

tools

directory. The command line, in this case, would look as
