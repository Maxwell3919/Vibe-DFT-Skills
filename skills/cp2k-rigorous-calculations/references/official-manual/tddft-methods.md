...
for corresponding excited-state gradients of

spin-conserving excitations and in

HernandezSegura2025

for excited-states energies and
...
. Please cite these papers if you were to use the TDDFT module for

the computation of excitation energies (
...
) or excited-state gradients

(

Hehn2022

) of spin-conserving excitations and
...
and the corresponding excited-state

eigenvectors

\(\mathbf{X}^p\)

as an Hermitian eigenvalue problem
...
contains as zeroth-order contributions the difference in the

Kohn-Sham (KS) orbital energies

\(\mathbf{F}\)

, and to first order kernel contributions

\(\mathbf{K}\)

which comprise –depending on the chosen density functional approximation – Coulomb
...
contributions as well as contributions due to the
...
denotes the atomic-orbital overlap matrix,

\(\mathbf{C}\)

the occupied ground-state KS
...
ground-state and response density
...
approximating both Coulomb and exchange contributions
...
semi-empirical operators

\(\boldsymbol{\gamma}^{\rm{\tiny{J}}}\)

and

\(\boldsymbol{\gamma}^{\rm{\tiny{K}}}\)

depending on the interatomic distance
...
that depend on the chemical hardness

\(\eta\)

, the Fock-exchange mixing parameter
...
for either Coulomb and exchange interactions.

Within the current implementation, oscillator strengths can be calculated for molecular systems in

the length form and for periodic systems using the velocity form (see
...
variational Lagrangian for each excited state
...
\(\bar{\mathbf{W}}^{\rm{\tiny{X}}}\)

,

\(\bar{\mathbf{W}}^{\rm{\tiny{C}}}\)

, and

\(\bar{\mathbf{Z}}\)

to ensure stationarity of the
...
and the underlying KS ground-state

reference has to be specified in the
...
: option for the kernel matrix

\(\mathbf{K}\)

to choose between the full kernel for GGA or hybrid functionals and the simplified

TDA kernel

NSTATES

: number of excitation energies to be
...
be optimized furthermore has to be specified by adding the subsection
...
use the Auxiliary Density Matrix Method (ADMM)

Guidon2010

to approximate the exact exchange

contribution. For GGAs, the corresponding ADMM sections are not required and it is sufficient to

specify the chosen functional in the subsection
...
(valid for fully periodic systems only),

LENGTH

(valid for molecular systems

only) and

VELOCITY

(both). When referring to the length form, the reference point to calculate
...
(user-defined point) and

ZERO

(origin of the coordinate system) for the
...
and the state to be optimized has to be specified in the

EXCITED_STATES

section. Note that the number of excited
...
needs to be chosen carefully and is really crucial to ensure a
...
relatively small amount of exchange

\(a_{\rm{\tiny{EX}}}=0.2 / 0.1\)

is therefore recommended.

For details on the STDA method see also
...
keyword to modify the parameter
...
reference GFN1-xTB. However, it is then recommended to adjust all parameters
...
oscillator strengths should be calculated using the
...
enables to adjust the number of unoccupied

orbitals. It is possible to print the NTOs as

CUBE_FILES

or in Molden format, with the latter
