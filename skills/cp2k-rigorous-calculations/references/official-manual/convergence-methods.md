...
program by default. This means that after reaching

MAX_SCF

cycles (default 50), the value printed under the

Convergence

column does not meet
...
and the other documentations under
...
specified by a pair of keywords
...
respectively, that should represent a

realistic electronic state. Use the
...
) to request for an unrestricted, spin-polarized calculation of

open-shell systems. For instance, it is well-known that the dioxygen molecule
...
ground state, and even the lowest

singlet

state is an excited state available from

energetic conditions like in photochemical systems; unless singlet oxygen is really intended, the

calculation of a dioxygen molecule should normally use
...
may not be enough; this is where a delicate preparation of SCF initial
...
section can be used to provide the orbital occupation
...
Setting SCF_GUESS to

RESTART

and specifying a wavefunction restart file for the keyword

WFN_RESTART_FILE_NAME

will instead parse the
...
After a ground-state calculation, use
...
a k-point sampling scheme: the former is typically
...
. They cannot be interchanged for the purpose of restarting.

Starting from CP2K version 2026.2, it is possible to produce a
...
section or the respective section of model Hamiltonian has been
...
, can also help convergence when chosen as
...
section, with full support for the

mixing and smearing techniques.

The mixing procedures of the density matrix in

MIXING

supports several methods in the keyword
...
which is very useful for systems with small to none

band gap and strong static correlation. The possibilities provided by the keyword

METHOD

include Fermi-Dirac distribution and several
...
systematically reduced to 0. Also note that some
...
Alternative to the diagonalization is the minimization-based orbital transformation (

OT

)

method, activated by setting the
...
section controls an outer loop where the
...
is met without satisfying

SCF/EPS_SCF

, the program leaves the inner loop, updates
