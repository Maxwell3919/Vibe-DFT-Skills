...
) for writing error messages. In linux systems, they are
...
. During program execution, message

passing is controlled by
...
A typical MPI-parallel CP2K run with the command below specifies the file
...
to which a majority of log data is written, and the command-line interface (CLI) screen as
...
With the following command, the file
...
from CP2K will be piped

to the linux

tee

utility, which prints log data to the CLI screen and the file
...
is still the CLI screen as it is not redirected or piped.
...
If CP2K is launched by job scheduling systems in a queue, the
...
with its own configurations; for instance, the Slurm Workload Manager defines
...
A CP2K input file usually contains some sections named

PRINT

that dictates whether and where a

chunk of information will be printed. Take

FORCE_EVAL/PRINT/FORCES

as an example, which prints
...
for “the screen or standard logger”, which is synonymous with the
...
in other ways will create a separate file to write the output.
...
output logs depending on the replica and parallelization in use; sometimes

the single primary log (specified by

-o

option above) may not be as informative or primitive as
...
whether it is still running with
...
commands or with the job scheduler.
...
in the timing, but may be confirmed in a re-run with the
...
commands monitoring the

memory consumption. If allocating more memory is not viable, reducing the number of MPI parallel

processes or switching to MPI + OpenMP hybrid parallelism for the

psmp

build can reduce the total

memory consumption as a compromise.

If CP2K is built with

ELPA

, it would be the default

diagonalization library. Setting
...
can probably circumvent some bugs like in github issue

#4484

.

When running a hybrid DFT calculation, where it is well-known that the evaluation of
...
and the in-core storage tends to be very
...
can help, and utilizing the latest
...
A normal MPI parallel execution started as follows will be shown as 4 processes in
...
reflecting the parallelization setup.
...
… along with many other interleaved and multiplicated lines like
...
, then CP2K is not parallelized as intended, but run as several
...
) is not the one that CP2K has been linked to (as shown by
...
files, modules, and conda envs. If the CP2K is built from source

with toolchain

, do not forget to source the

cp2k_env

to load the dependencies as instructed.
...
cannot be fitted in. For example, the edit descriptor

F12.6

specifies a real float-type field with
...
would be shown instead. Most of these field formats are designed with
...
Beyond that, abnormal values may be represented as
...
mechanisms in CP2K intended for
...
is a minimal blanket statement for these unlikely events, only indicating the code
...
occur for a valid input file strictly following documentation, report to
...
detected while building a neighbor list. Although the keyword
...
in some error messages such as
...
options should be set correctly based on the
...
This error arises from a combination of high
