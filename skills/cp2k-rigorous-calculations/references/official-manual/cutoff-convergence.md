...
, as with nearly all ab initio Density Functional Theory simulation packages, requires

the use of a real-space (RS) integration grid to represent certain functions, such as the electron

density and the product Gaussian functions.

QUICKSTEP

uses a multi-grid system for mapping the
...
. The number of levels for the multi-grid is

defined by

NGRIDS

, and by default this is set to 4. The

keyword

CUTOFF

defines the planewave cutoff (default unit

is in Ry) for the

finest

level of the multi-grid. The higher the planewave cutoff, the finer the
...
has a default value of 3.0, and since

CP2K

versions 2.0, can be configured by the
...
then needs to map the Gaussians onto the grids. The

keyword

REL_CUTOFF

controls which product Gaussians

are mapped onto which level of the multi-grid.
...
defines the planewave cutoff of a

reference grid covered by a Gaussian with unit standard deviation

\((e^{\mathrm{|\overrightarrow{r}|^{2}}})\)

. A Gaussian is mapped onto the coarsest level of the
...
will cover on a reference grid defined
...
is too low, then all grids will be coarse and the

calculation may become inaccurate; and if

REL_CUTOFF

is too low, then even if you have a high
...
values which are sufficient for a given

accuracy (say,

\(10^{−6}\)

Ry in total energy), we need to perform a series of single point energy
...
tutorial on how to perform a simple static energy and force calculation

using

QUICKSTEP

should have no trouble in understanding most parts the above input.
...
, so that the information about how many
...
are markers, which the automated scripts will search for
...
number, and systematically vary
...
to 60 Ry is usually sufficient for most
...
We want to perform a series of calculations, with
...
pseudopotential parameter files
...
) are in the working directory together with

template.inp

, then one can write a

bash script, such as the file
...
is set to the respective values in the range
...
be used in parallel for each job.

no_proc_to_use

sets the total number of processors to be used
...
to not assign each calculation to the same two

processors. Note that when using a Resource Manager for submitting jobs,
...
of 16 cores are used for performing the

CUTOFF

convergence test calculations, and 2 cores are used for each calculation. This means up to 8 jobs

will run in parallel, until the jobs are exhausted from the list given in
...
distribution of Gaussians on the multi-grid are written in the
...
of 60 Ry, 2720 product Gaussians has been
...
plot the results. This can be done, again, by using a simple script.
...
to 250 Ry and above would give an

error in total energy less than

\(10^{−8}\)

Ha. The reader may also notice that as

CUTOFF

increases, the number of Gaussians being assigned
...
may eventually lead to a slow convergence
...
, as the total energy is converged, and the
...
In the next step, we vary the value of
...
, we follow a similar procedure as that for

CUTOFF

. Using the same template input file
...
is set to 10, 20, …, 100 respectively.
...
In the above example, again, we have used 16 cores in total, and with each job using 2
...
from the results as that for the

CUTOFF

calculations.

To put all of the results from the

REL_CUTOFF

calculations in one place, we can make some minor modifications to
...
, more Gaussians get mapped onto the finer

grids. The error in total energy reduces to less than
...
is greater or equal to 60 Ry. The results
