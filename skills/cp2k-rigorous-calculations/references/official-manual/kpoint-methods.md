...
in the Brillouin zone. Quantities such as the electronic density, total energy, and
...
are normalized k-point weights.

A Gamma-only calculation samples only

\(\mathbf{k}=0\)

. It is often appropriate for isolated systems,
...
quantities with the k-point weights. The

&KPOINTS

section therefore describes the sampling used
...
gives the usual Gamma-only calculation (
...
. Real wavefunctions are only valid
...
The following table summarizes selected common cases for

DFT%KPOINTS

; it is not exhaustive. Refer

to the documentation of the relevant feature for detailed requirements and limitations or see issue

#4854

which tracks the current status of k-points
...
No k-point path available.

Atomic symmetry reduction

Experimental.

Validate against an equivalent unreduced calculation.
...
Independent-particle response only (
...
Support with separate workflow.
...
selects one of the schemes described below.
...
For the conventional Gamma-only calculation, omit
...
accepts an explicitly supplied weighted set of k-points:
...
line contains three coordinates and one
...
defines an integration set for the SCF calculation. It is not the usual interface

for a high-symmetry band path. Use
...
selects the smallest valid number of processes per group.
...
explains DOS and PDOS output. DOS/PDOS commonly need a denser mesh
...
documents RI-HFX with k-point sampling and includes a band-structure

example.

Wannier90 interface

documents the experimental Wannier90 interface, including

export from the SCF k-point mesh and the treatment of symmetry-reduced SCF meshes.

Geometry and cell optimization

describes geometry and cell optimization. Their
...
k-space inversion (time-reversal) reduction
...
, which uses additional operations that map the
...
keyword controls the second level. It is

off by default; this does

not

disable the default time-reversal reduction for regular meshes.
...
points. This is the standard reduction path when both
...
is present in a shared input template, but only time-reversal reduction is
...
preserves the supplied list by default. When inversion-only reduction is requested

for an explicit list, each
...
pair must be present with equal weights.
...
For Monkhorst–Pack and MacDonald meshes,
...
symmetry reduction but retains k-space inversion (time-reversal) reduction. Use both
...
to obtain a strict full-mesh reference calculation.
...
current atomic structure. Enable it with
...
, provided that all explicit weights are equal and that the complete set

is closed under every requested symmetry operation. A nonuniform

GENERAL

list, including a band
...
is the established default backend. The

optional

SPGLIB

backend uses symmetry operations returned by spglib, including fractional
...
, molecular dynamics, and related calculations, CP2K determines atomic
...
list must remain symmetry-closed at every step or it is rejected.
...
prints k-point information and is
