...
Most GPW calculations in CP2K use norm-conserving Goedecker-Teter-Hutter (
...
, and the actual potential is

selected for each atomic
...
, for example, means that six valence electrons are treated
...
is a natural choice for PBE calculations with oxygen. Mixing
...
contains widely used GTH potentials for common GPW calculations.

POTENTIAL_UZH

contains the UZH protocol GTH potentials designed to be used with matching UZH
...
contain more specialized potentials.

ECP_POTENTIALS

contains effective core potentials for Gaussian integral based calculations.

For new GPW production inputs, prefer a matching UZH protocol pair from
...
when it is available for the element and functional family. The older

GTH_POTENTIALS

library remains important for reproducing established calculations and for cases

where a matching UZH setup is not available.

For all-electron calculations, use

POTENTIAL

ALL

together with an all-electron basis set and the
