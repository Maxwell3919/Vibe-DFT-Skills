...
Which angular-momentum channels dominate a feature, for example transition-metal
...
states, or adsorbate frontier orbitals?

How strong is orbital hybridization, such as metal–oxygen
...
subsection for broadened, directly plottable

DOS/PDOS curves, is only available in CP2K 2026.2 and later versions. The ordinary
...
outputs keep the traditional non-broadened data, while
...
. Requesting this print section writes the

total DOS. Set

&PDOS

under the same section to
...
: choose the energy-grid spacing.

NLUMO

: requested number of unoccupied states for

DOS/PDOS.

&CURVE

: request additional broadened, directly

plottable DOS/PDOS curve output.

ENERGY_UNIT

: print curve energies in

Hartree or eV.

ENERGY_ZERO

: choose the reference energy

for curve output.

&BROADEN

: choose the line shape and

broadening width for curve output.

&PDOS

: request projected DOS on each element.
...
: request atom-list or real-space local
...
: split angular-momentum

channels into individual components. This keyword also applies to
...
is normally the appropriate zero for metals and smeared calculations.

HOCO

places the highest occupied crystal orbital at zero. It is often more convenient for
...
prints the absolute Kohn-Sham eigenvalues.
...
when smearing or fractional occupations are present, and
...
, CP2K skips the broadened curve output and prints a
...
, CP2K writes projected DOS, usually one file per atomic kind and spin channel. The

ordinary

.pdos

files contain state-resolved projected weights. If

&CURVE

is also requested, CP2K

additionally writes broadened PDOS curve files with a projected
...
column in one PDOS curve file is the projected total for that atomic kind, not the total
...
transition-metal compounds: inspect the relative positions and spin splitting of metal
...
calculations, the molecular-orbital coefficients

C

are first transformed with the square root of
...
and the projected weight of a state is accumulated from
...
The broadened DOS and PDOS curve files can be plotted directly with

gnuplot

,

since comment lines starting with

#

are ignored. For example, if the first column is the energy
...
The plotted energy range can be restricted directly in the

plot

command. For example, to show only
...
When comparing DOS/PDOS curves with band structures, use the same energy zero, for example
...
for many insulating and molecular systems.
...
projects onto explicitly listed atoms.

&R_LDOS

projects onto a real-space volume defined
...
in CP2K is different from the real-space local density of states

\(\rho(\mathbf{r}, E)\)

often used in, for example, STM-related analysis. In this DOS/PDOS print

section,

LDOS

denotes a projected DOS for user-defined lists of atoms, i.e. a local PDOS over

selected atoms.

R_LDOS

further restricts the analysis to a user-defined spatial region around
...
XAS and XAS_TDP have their own dedicated

&PDOS

print

sections. They are intentionally separate from the DFT
...
For XAS_TDP, the distinction is stronger. The

.pdos

files reuse the standard PDOS projection and
...
remain useful plotting controls, while ordinary DFT-DOS concepts such as
