...
```bash
python3 tools/sync_official_manual_cache.py --refresh --skill qe-rigorous-calculations
...
```bash
python3 skills/qe-rigorous-calculations/scripts/sync_official_manuals.py --refresh
python3 skills/qe-rigorous-calculations/scripts/sync_official_manuals.py --check
```
...
Read the generated `index.md` under `${XDG_CACHE_HOME:-$HOME/.cache}/vibe-dft-skills/official-manuals/qe-rigorous-calculations/`. Keep the official URL, authority/version identity, and local conversion label in every claim. A cache pass proves document identity and readability only; it does not prove executable behavior, convergence, or physical validity.

This Skill has 35 registered source records across these authorities:
